...
protected:
    ...
    ListBuffer(ListBuffer<T> &&original);
...
template <typename T>
ListBuffer<T>::Node::Node(T data) noexcept : data(data), next(nullptr) {}
...
ListBuffer<T>& ListBuffer<T>::operator = (const ListBuffer<T> &other)
{
    ...
    ListBuffer<T> temp;
    temp.head = (other.length_ == 0) ? nullptr : new Node(other.head -> data);
    Node *currentOther = other.head;
    Node *currentNode = temp.head;
    for (std::size_t i = 1; i < other.length_; ++i)
    {
        currentOther = currentOther -> next;
        currentNode -> next = new Node(currentOther -> data);
        currentNode = currentNode -> next;
    }
    temp.tail = currentNode;
    temp.length_ = other.length_;
